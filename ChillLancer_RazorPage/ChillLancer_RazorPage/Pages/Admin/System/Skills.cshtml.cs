...
namespace ChillLancer_RazorPage.Pages.Admin.System
{
    public class SkillsModel : PageModel
    {
        ...
        public SkillsModel(IHttpClientFactory httpClientFactory, IConfiguration configuration)
        {
            _httpClient = httpClientFactory.CreateClient();
            _configuration = configuration;
        }

        public List<SkillVM> SkillsList { get; set; } = [];

        [BindProperty]
        public SkillVM SkillObj { get; set; } = new SkillVM();
        ...
        public async Task OnGetAsync()
        {
            ...
            var apiUrl = $"{serverUrl}/api/skill";

            try
            {
                var response = await _httpClient.GetFromJsonAsync<List<SkillVM>>(apiUrl);
                if (response != null)
                {
                    SkillsList = response;
                }
        ...
        public async Task<IActionResult> OnPostSaveAsync()
        {
            ...
            if (!IsEdit)
            {
                var response = await _httpClient.PostAsJsonAsync(apiUrl, SkillObj);
            ...
            else
            {
                ...
                {
                    Content = JsonContent.Create(SkillObj)
                };
